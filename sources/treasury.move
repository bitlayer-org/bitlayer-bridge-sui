...
module bridge::treasury {
    ...
    public(package) fun add_new_token(
        ...
        if (!native_token){
            ...
            assert!(self.waiting_room.contains<String>(token_name), EUnsupportedTokenType);
            ...
            if (!self.supported_tokens.contains<TypeName, BridgeTokenMetadata>(&type_name)) {
    ...
    public fun is_valid_token_id<T>(self: &BridgeTreasury, _token_id: u8): bool {
      let coin_type = type_name::get<T>();
      let token_type = self.id_token_type_map.try_get(&_token_id);
      if (token_type.is_some()) {
        token_type.destroy_some() == coin_type
      } else {
        false
      }
    }
