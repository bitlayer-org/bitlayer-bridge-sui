...
module bridge::message {
    ...
    public struct AddRoutesOnSui has drop {
    ...
    public fun extract_blocklist_payload(message: &BridgeMessage): Blocklist {
        ...
        let mut committee_eth_addresses = vector[];
        while (address_count > 0) {
            ...
            committee_eth_addresses.push_back(address);
    ...
    public fun extract_add_routes_on_sui(message: &BridgeMessage): AddRoutesOnSui {
        let mut bcs = bcs::new(message.payload);
        let supported_chain_ids = bcs.peel_vec_u8();
        let supported_token_ids = bcs.peel_vec_u8();
    ...
    public fun create_token_bridge_message(
        ...
        // chain_ids::is_valid_route(self, destination, token)(target_chain);
    ...
    /// [blocklist_type: u8]
    /// [committee_length: u8]
    /// [committee_ecdsa_addresses: byte[][]]
    public fun create_blocklist_message(
        source_chain: u8,
        seq_num: u64,
        // 0: block, 1: unblock
        blocklist_type: u8,
        committee_ecdsa_addresses: vector<vector<u8>>,
    ): BridgeMessage {
        chain_ids::assert_valid_chain_id(source_chain);

        let address_length = committee_ecdsa_addresses.length();
        let mut payload = vector[blocklist_type, (address_length as u8)];
        let mut i = 0;

        while (i < address_length) {
            let address = committee_ecdsa_addresses[i];
    ...
    public fun create_update_bridge_limit_message(
        ...
        chain_ids::assert_valid_chain_id(source_chain_id);
    ...
    /// [bridge_amounts:vector<u64>]
    /// [supporteds:vector<bool>]
    public fun create_add_routes_on_sui_message(
        source_chain: u8,
        seq_num: u64,
        supported_chain_ids: vector<u8>,
        supported_token_ids: vector<u8>,
        ...
        payload.append(bcs::to_bytes(&supported_token_ids));
    ...
    public fun blocklist_committee_addresses(self: &Blocklist): &vector<vector<u8>> {
        &self.committee_eth_addresses
    }
    ...
    public fun update_bridge_limit_payload_sending_token(self: &UpdateBridgeLimit): u8 {
        self.sending_token
    }
    ...
    public fun supported_chain_ids(self: &AddRoutesOnSui): vector<u8> {
    ...
    public fun bridge_amounts(self: &AddRoutesOnSui): vector<u64> {
        self.bridge_amounts
    }

    public fun supporteds(self: &AddRoutesOnSui): vector<bool> {
        self.supporteds
    }
