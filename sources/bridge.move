...
module bridge::bridge {
    ...
    use bridge::chain_ids::{Self, BridgeSupportedRoutes};
    ...
    public struct BridgeInner has store {
        ...
        submitters: VecMap<address, bool>,
    ...
    public struct TokenWithdrawEvent has copy, drop {
        seq_num: u64,
        source_chain: u8,
    ...
    const ETokenNotSupported: u64 = 20;
    const ESenderNotSubmitter: u64 = 21;
    ...
    public struct BridgeVersionUpdated has copy, drop {
        operator: address,
        old_version: u64,
        new_version: u64,
    }
    ...
    public struct UpdateBridgeSubmitter has copy, drop {
        submitter: address,
        active: bool,
    }

    public struct UpdateBridgeFeeRecipient has copy, drop {
        fee_recipient: address,
    }
    ...
    public fun update_fee_recipient(
        ...
        assert_admin_version(admin_cap);
        let inner = load_inner_mut(bridge);
        inner.fee_recipient = fee_recipient;
        emit(UpdateBridgeFeeRecipient{fee_recipient});
    }

    public fun update_submitter(
        bridge: &mut Bridge,
        admin_cap: &AdminCap,
        submitter: address,
        active: bool,
        ...
        assert_admin_version(admin_cap);
        let inner = load_inner_mut(bridge);
        if (inner.submitters.contains(&submitter)) {
            let _active = inner.submitters.get_mut(&submitter);
            *_active = active;
        }else {
            inner.submitters.insert(submitter, active);
        };
        emit(UpdateBridgeSubmitter{submitter, active});
    }

    public fun create_bridge_committee(
        bridge: &mut Bridge,
        admin_cap: &AdminCap,
        ...
        assert_admin_version(admin_cap);
        bridge.init_bridge_committee(active_committee_powers, min_stake_participation_percentage, ctx)
    }
    ...
    public fun create_bridge_committee_by_vec(
        bridge: &mut Bridge,
        admin_cap: &AdminCap,
        active_committees: vector<vector<u8>>,
        active_powers: vector<u64>,
        min_stake_participation_percentage: u64,
        ctx: &TxContext
    ) {
        assert_admin_version(admin_cap);
        bridge.init_bridge_committee_by_vec(active_committees, active_powers, min_stake_participation_percentage, ctx)
    }

    fun init_bridge_committee_by_vec(
        bridge: &mut Bridge,
        active_committees: vector<vector<u8>>,
        active_powers: vector<u64>,
        min_stake_participation_percentage: u64,
    ...
    public fun committee_registration(
        ...
        load_inner_mut(bridge)
            .committee
            .register(bridge_member_address_vec);
    }
    ...
    public fun register_foreign_token<T>(
        ...
        load_inner_mut(bridge)
            .treasury
            .register_foreign_token<T>(tc,  metadata)
    }
    ...
    public fun send_token<T>(
        ...
        // assert!(inner.routes.is_valid_route(target_chain, target_token_id), EInvalidBridgeRoute);
        assert!(target_address.length() == EVM_ADDRESS_LENGTH, EInvalidEvmAddress);
        assert!(inner.treasury.is_valid_token_id<T>(target_token_id), ETokenNotSupported);

        let route = inner.routes.get_route(target_chain, target_token_id);
        ...
        if (fees > 0) {
            let fees_balance = balance::split(&mut token_balance, fees);
            let fee_coin = coin::from_balance(fees_balance, ctx);
            transfer::public_transfer(fee_coin, inner.fee_recipient);
        };

        let amount = token_balance.value();
        assert!(original_amount > 0, ETokenValueIsZero);
        ...
        let burn_coin = coin::from_balance(token_balance, ctx);
        inner.treasury.burn(burn_coin);

        // Update bridge amount
        let routes = &mut inner.routes;
        routes.update_bridge_amount(&route, amount, false);
    ...
    public fun approve_token_transfer(
        ...
        // verify sender is a submitter
        assert!(inner.submitters.contains(&ctx.sender()), ESenderNotSubmitter);
    ...
    fun claim_token_internal<T>(
        ...
        let target_chain = token_payload.token_target_chain();
        let token_type = token_payload.token_type();
        ...
        let route = inner.routes.get_route( source_chain, token_type);
        // check token type
        assert!(
            treasury::is_valid_token_id<T>(&inner.treasury, token_type),
        ...
        // Update bridge amount
        let routes = &mut inner.routes;
        routes.update_bridge_amount(&route, amount, true);
    ...
    fun execute_update_bridge_limit(inner: &mut BridgeInner, payload: UpdateBridgeLimit) {
        let sending_chain = payload.update_bridge_limit_payload_sending_chain();
        ...
        let route = inner.routes.get_route(
            ...
            payload.update_bridge_limit_payload_sending_token()
        );
    ...
    fun execute_add_routes_on_sui(inner: &mut BridgeInner, payload: AddRoutesOnSui) {
        // FIXME: assert native_token to be false and add test
        // let native_token = payload.is_native();
        let mut chain_ids = payload.supported_chain_ids();
        let mut token_ids = payload.supported_token_ids();
    ...
    public fun get_current_seq_num(bridge: &Bridge, msg_type: u8): u64 {
        let inner = load_inner(bridge);
        let num = inner.sequence_nums.try_get(&msg_type);
        if (num.is_some()) {
          num.destroy_some()
        }else {
          0
    ...
    public fun assert_admin_version(admin_cap: &AdminCap) {
        assert!(admin_cap.version == CURRENT_VERSION, EWrongAdminCapVersion);
    }

    public fun migrate_admin_cap_version(admin_cap: &mut AdminCap, ctx: &TxContext) {
        assert!(admin_cap.version < CURRENT_VERSION, EVersionNoUpdateRequired);
        let old_version = admin_cap.version;
        admin_cap.version = CURRENT_VERSION;
        emit(AdminCapVersionUpdated{
    ...
    public fun migrate_bridge_version(admin_cap: &mut AdminCap, bridge: &mut Bridge, ctx: &TxContext) {
        assert_admin_version(admin_cap);
        
        let version = bridge.inner.version();
        assert!(version < CURRENT_VERSION, EVersionNoUpdateRequired);
        let mut inner: BridgeInner;
        let cap: VersionChangeCap;
        (inner, cap) = bridge.inner.remove_value_for_upgrade();
        let old_version = inner.bridge_version;
        inner.bridge_version = CURRENT_VERSION;

        bridge.inner.upgrade(CURRENT_VERSION, inner, cap);
        emit(BridgeVersionUpdated{
            old_version: old_version, 
            new_version: CURRENT_VERSION, 
            operator: ctx.sender(),
        });
    ...
    #[test_only]
    public fun create_bridge_for_testing(chain_id: u8, ctx: &mut TxContext) {
        create(chain_id, ctx);
    }
    ...
    #[test_only]
    public fun inner_routes(bridge_inner: &BridgeInner): &BridgeSupportedRoutes {
        &bridge_inner.routes
    }
    ...
    #[test_only]
    public fun test_execute_add_routes(
        inner: &mut BridgeInner,
        payload: AddRoutesOnSui,
    ) {
        execute_add_routes_on_sui(inner, payload)
    }
    ...
    #[test_only]
    public fun unwrap_withdraw_event(event: TokenWithdrawEvent): (u64, u8, vector<u8>, u8, vector<u8>, u8, u64) {
        (
            event.seq_num,
            event.source_chain,
