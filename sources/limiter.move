...
module bridge::limiter {
    ...
    // const USD_VALUE_MULTIPLIER: u64 = 10000000000; // 10 DP accuracy
    ...
    public(package) fun update_route_limit(
        ...
        let sending_chain = *route.destination();
        let sending_token = *route.token();
    ...
    public(package) fun unpack_route_limit_event(event: UpdateRouteLimitEvent):
        (u8, u8, u64) 
    {
        (event.sending_chain, event.sending_token, event.new_limit)
    }
}
